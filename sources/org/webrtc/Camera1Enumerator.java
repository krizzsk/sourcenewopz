...
public class Camera1Enumerator implements CameraEnumerator {
    ...
    public String[] getDeviceNames() {
        ...
        for (int i = 0; i < Camera.getNumberOfCameras(); i++) {
            ...
            if (deviceName != null) {
                ...
                Logging.m3800d(TAG, "Index: " + i + ". " + deviceName);
            } else {
                Logging.m3801e(TAG, "Index: " + i + ". Failed to query camera name.");
            }
        }
    ...
    private static Camera.CameraInfo getCameraInfo(int i) {
        ...
        try {
            ...
            Logging.m3802e(TAG, "getCameraInfo failed on index " + i, e);
            return null;
        }
    }
    ...
    private static List<CameraEnumerationAndroid.CaptureFormat> enumerateFormats(int i) {
        int i2;
        Logging.m3800d(TAG, "Get supported formats for camera index " + i + ".");
        long elapsedRealtime = SystemClock.elapsedRealtime();
        Camera camera = null;
        try {
            Logging.m3800d(TAG, "Opening camera with index " + i);
            ...
            try {
                ...
                Logging.m3802e(TAG, "getSupportedFormats() failed on camera index " + i, e);
            }
            long elapsedRealtime2 = SystemClock.elapsedRealtime();
            Logging.m3800d(TAG, "Get supported formats for camera index " + i + " done. Time spent: " + (elapsedRealtime2 - elapsedRealtime) + " ms.");
            return arrayList;
        } catch (RuntimeException e2) {
            Logging.m3802e(TAG, "Open camera failed on camera index " + i, e2);
    ...
    static int getCameraIndex(String str) {
        Logging.m3800d(TAG, "getCameraIndex: " + str);
        for (int i = 0; i < Camera.getNumberOfCameras(); i++) {
