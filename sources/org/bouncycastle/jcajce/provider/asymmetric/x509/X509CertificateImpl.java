...
import org.bouncycastle.jcajce.p078io.OutputStreamFactory;
...
abstract class X509CertificateImpl extends X509Certificate implements BCX509Certificate {
    ...
    private void checkSignature(PublicKey publicKey, Signature signature, ASN1Encodable aSN1Encodable, byte[] bArr) throws CertificateException, NoSuchAlgorithmException, SignatureException, InvalidKeyException {
        if (isAlgIdEqual(this.f6242c.getSignatureAlgorithm(), this.f6242c.getTBSCertificate().getSignature())) {
            ...
            try {
                ...
                this.f6242c.getTBSCertificate().encodeTo(bufferedOutputStream, ASN1Encoding.DER);
    ...
    private void doVerify(PublicKey publicKey, SignatureCreator signatureCreator) throws CertificateException, NoSuchAlgorithmException, InvalidKeyException, SignatureException, NoSuchProviderException {
        ...
        int i = 0;
        if (z && X509SignatureUtil.isCompositeAlgorithm(this.f6242c.getSignatureAlgorithm())) {
            List<PublicKey> publicKeys = ((CompositePublicKey) publicKey).getPublicKeys();
            ASN1Sequence instance = ASN1Sequence.getInstance(this.f6242c.getSignatureAlgorithm().getParameters());
            ASN1Sequence instance2 = ASN1Sequence.getInstance(DERBitString.getInstance(this.f6242c.getSignature()).getBytes());
            boolean z2 = false;
        ...
        } else if (X509SignatureUtil.isCompositeAlgorithm(this.f6242c.getSignatureAlgorithm())) {
            ASN1Sequence instance4 = ASN1Sequence.getInstance(this.f6242c.getSignatureAlgorithm().getParameters());
            ASN1Sequence instance5 = ASN1Sequence.getInstance(DERBitString.getInstance(this.f6242c.getSignature()).getBytes());
            boolean z3 = false;
            ...
            Signature createSignature = signatureCreator.createSignature(X509SignatureUtil.getSignatureName(this.f6242c.getSignatureAlgorithm()));
            if (z) {
                List<PublicKey> publicKeys2 = ((CompositePublicKey) publicKey).getPublicKeys();
                while (i != publicKeys2.size()) {
                    try {
                        checkSignature(publicKeys2.get(i), createSignature, this.f6242c.getSignatureAlgorithm().getParameters(), getSignature());
                        return;
            ...
            checkSignature(publicKey, createSignature, this.f6242c.getSignatureAlgorithm().getParameters(), getSignature());
        }
    }
    ...
    public void checkValidity(Date date) throws CertificateExpiredException, CertificateNotYetValidException {
        if (date.getTime() > getNotAfter().getTime()) {
            throw new CertificateExpiredException("certificate expired on " + this.f6242c.getEndDate().getTime());
        } else if (date.getTime() < getNotBefore().getTime()) {
            throw new CertificateNotYetValidException("certificate not valid till " + this.f6242c.getStartDate().getTime());
        }
    }
    ...
    public Set getCriticalExtensionOIDs() {
        ...
        Extensions extensions = this.f6242c.getTBSCertificate().getExtensions();
        if (extensions == null) {
            return null;
        }
    ...
    public List getExtendedKeyUsage() throws CertificateParsingException {
        byte[] extensionOctets = getExtensionOctets(this.f6242c, "2.5.29.37");
        if (extensionOctets == null) {
            return null;
        }
        try {
    ...
    public byte[] getExtensionValue(String str) {
        ASN1OctetString extensionValue = getExtensionValue(this.f6242c, str);
        if (extensionValue == null) {
            return null;
        }
        try {
    ...
    public Collection getIssuerAlternativeNames() throws CertificateParsingException {
        return getAlternativeNames(this.f6242c, Extension.issuerAlternativeName.getId());
    }

    public Principal getIssuerDN() {
        return new X509Principal(this.f6242c.getIssuer());
    }

    public boolean[] getIssuerUniqueID() {
        DERBitString issuerUniqueId = this.f6242c.getTBSCertificate().getIssuerUniqueId();
    ...
    public X500Name getIssuerX500Name() {
        return this.f6242c.getIssuer();
    }

    public X500Principal getIssuerX500Principal() {
        try {
            return new X500Principal(this.f6242c.getIssuer().getEncoded(ASN1Encoding.DER));
    ...
    public Date getNotAfter() {
        return this.f6242c.getEndDate().getDate();
    }

    public Date getNotBefore() {
        return this.f6242c.getStartDate().getDate();
    }

    public PublicKey getPublicKey() {
        try {
            return BouncyCastleProvider.getPublicKey(this.f6242c.getSubjectPublicKeyInfo());
        } catch (IOException unused) {
            return null;
        }
    }
    ...
    public BigInteger getSerialNumber() {
        return this.f6242c.getSerialNumber().getValue();
    }
    ...
    public String getSigAlgOID() {
        return this.f6242c.getSignatureAlgorithm().getAlgorithm().getId();
    }
    ...
    public byte[] getSignature() {
        return this.f6242c.getSignature().getOctets();
    }

    public Collection getSubjectAlternativeNames() throws CertificateParsingException {
        return getAlternativeNames(this.f6242c, Extension.subjectAlternativeName.getId());
    }

    public Principal getSubjectDN() {
        return new X509Principal(this.f6242c.getSubject());
    }

    public boolean[] getSubjectUniqueID() {
        DERBitString subjectUniqueId = this.f6242c.getTBSCertificate().getSubjectUniqueId();
    ...
    public X500Name getSubjectX500Name() {
        return this.f6242c.getSubject();
    }

    public X500Principal getSubjectX500Principal() {
        try {
            return new X500Principal(this.f6242c.getSubject().getEncoded(ASN1Encoding.DER));
        } catch (IOException unused) {
    ...
    public byte[] getTBSCertificate() throws CertificateEncodingException {
        try {
            return this.f6242c.getTBSCertificate().getEncoded(ASN1Encoding.DER);
        } catch (IOException e) {
            throw new CertificateEncodingException(e.toString());
        }
    }
    ...
    public TBSCertificate getTBSCertificateNative() {
        return this.f6242c.getTBSCertificate();
    }

    public int getVersion() {
        return this.f6242c.getVersionNumber();
    }

    public boolean hasUnsupportedCriticalExtension() {
        Extensions extensions;
        if (getVersion() != 3 || (extensions = this.f6242c.getTBSCertificate().getExtensions()) == null) {
            return false;
        }
        Enumeration oids = extensions.oids();
        while (oids.hasMoreElements()) {
