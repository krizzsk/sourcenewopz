...
final class CctTransportBackend implements TransportBackend {
    ...
    private static int getPackageVersionCode(Context context) {
        try {
            ...
            Logging.m37340e(LOG_TAG, "Unable to find version code for package", e);
            return -1;
        }
    }
    ...
    private BatchedLogRequest getRequestBody(BackendRequest backendRequest) {
        ...
        for (Map.Entry entry : hashMap.entrySet()) {
            ...
            for (EventInternal eventInternal2 : (List) entry.getValue()) {
                ...
                if (encoding.equals(Encoding.m37334of("proto"))) {
                    builder = LogEvent.protoBuilder(encodedPayload.getBytes());
                } else if (encoding.equals(Encoding.m37334of("json"))) {
                    builder = LogEvent.jsonBuilder(new String(encodedPayload.getBytes(), Charset.forName("UTF-8")));
                } else {
                    Logging.m37342w(LOG_TAG, "Received event of unsupported encoding %s. Skipping...", encoding);
                }
    ...
    public HttpResponse doSend(HttpRequest httpRequest) throws IOException {
        ...
        Logging.m37337d(LOG_TAG, "Making request to: %s", (Object) httpRequest.url);
        ...
        try {
            ...
            try {
                ...
                Logging.m37341i(LOG_TAG, "Status Code: " + responseCode);
                Logging.m37341i(LOG_TAG, "Content-Type: " + httpURLConnection.getHeaderField("Content-Type"));
                Logging.m37341i(LOG_TAG, "Content-Encoding: " + httpURLConnection.getHeaderField("Content-Encoding"));
            ...
            Logging.m37340e(LOG_TAG, "Couldn't open connection, returning with 500", e);
            return new HttpResponse(500, (URL) null, 0);
        } catch (EncodingException | IOException e2) {
            Logging.m37340e(LOG_TAG, "Couldn't encode request, returning with 400", e2);
    ...
    public BackendResponse send(BackendRequest backendRequest) {
        ...
        try {
            ...
            if (httpResponse.code == 200) {
                return BackendResponse.m37335ok(httpResponse.nextRequestMillis);
            }
            ...
            Logging.m37340e(LOG_TAG, "Could not make request to the backend", e);
            return BackendResponse.transientError();
        }
    }
    ...
    static /* synthetic */ HttpRequest lambda$send$0(HttpRequest httpRequest, HttpResponse httpResponse) {
        ...
        Logging.m37337d(LOG_TAG, "Following redirect to: %s", (Object) httpResponse.redirectUrl);
