...
public class WebRtcAudioManager {
    ...
    public static synchronized void setStereoOutput(boolean z) {
        synchronized (WebRtcAudioManager.class) {
            Logging.m3804w(TAG, "Overriding default output behavior: setStereoOutput(" + z + VersionRange.RIGHT_OPEN);
            useStereoOutput = z;
        }
    }
    ...
    public static synchronized void setStereoInput(boolean z) {
        synchronized (WebRtcAudioManager.class) {
            Logging.m3804w(TAG, "Overriding default input behavior: setStereoInput(" + z + VersionRange.RIGHT_OPEN);
            useStereoInput = z;
        }
    }
    ...
    private static class VolumeLogger {
        ...
        private class LogVolumeTask extends TimerTask {
            ...
            public void run() {
                ...
                if (mode == 1) {
                    Logging.m3800d(WebRtcAudioManager.TAG, "STREAM_RING stream volume: " + VolumeLogger.this.audioManager.getStreamVolume(2) + " (max=" + this.maxRingVolume + ")");
                } else if (mode == 3) {
                    Logging.m3800d(WebRtcAudioManager.TAG, "VOICE_CALL stream volume: " + VolumeLogger.this.audioManager.getStreamVolume(0) + " (max=" + this.maxVoiceCallVolume + ")");
                }
            }
        }
    ...
    private boolean init() {
        Logging.m3800d(TAG, IPixView.PAGE_STATUS_INIT + WebRtcAudioUtils.getThreadInfo());
        if (this.initialized) {
            return true;
        }
        Logging.m3800d(TAG, "audio mode is: " + WebRtcAudioUtils.modeToString(this.audioManager.getMode()));
    ...
    private void dispose() {
        Logging.m3800d(TAG, "dispose" + WebRtcAudioUtils.getThreadInfo());
        if (this.initialized) {
    ...
    private boolean isDeviceBlacklistedForOpenSLESUsage() {
        ...
        if (z) {
            Logging.m3800d(TAG, Build.MODEL + " is blacklisted for OpenSL ES usage!");
        }
        return z;
    }
    ...
    private boolean isAAudioSupported() {
        Logging.m3804w(TAG, "AAudio support is currently disabled on all devices!");
        return false;
    }

    private int getNativeOutputSampleRate() {
        if (WebRtcAudioUtils.runningOnEmulator()) {
            Logging.m3800d(TAG, "Running emulator, overriding sample rate to 8 kHz.");
            return 8000;
        } else if (WebRtcAudioUtils.isDefaultSampleRateOverridden()) {
            Logging.m3800d(TAG, "Default sample rate is overriden to " + WebRtcAudioUtils.getDefaultSampleRateHz() + " Hz");
            return WebRtcAudioUtils.getDefaultSampleRateHz();
        } else {
            int sampleRateForApiLevel = getSampleRateForApiLevel();
            Logging.m3800d(TAG, "Sample rate is set to " + sampleRateForApiLevel + " Hz");
