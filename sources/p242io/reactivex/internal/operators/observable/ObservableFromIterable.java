...
public final class ObservableFromIterable<T> extends Observable<T> {
    ...
    /* renamed from: a */
    final Iterable<? extends T> f58832a;

    public ObservableFromIterable(Iterable<? extends T> iterable) {
        this.f58832a = iterable;
    }

    public void subscribeActual(Observer<? super T> observer) {
        try {
            Iterator<? extends T> it = this.f58832a.iterator();
            try {
                if (!it.hasNext()) {
    ...
    static final class FromIterableDisposable<T> extends BasicQueueDisposable<T> {
        ...
        /* renamed from: it */
        final Iterator<? extends T> f58833it;
        ...
        public void run() {
            while (!isDisposed()) {
                try {
                    this.downstream.onNext(ObjectHelper.requireNonNull(this.f58833it.next(), "The iterator returned a null value"));
                    if (!isDisposed()) {
                        try {
                            if (!this.f58833it.hasNext()) {
                                if (!isDisposed()) {
        ...
        public T poll() {
            ...
            if (!this.checkNext) {
                this.checkNext = true;
            } else if (!this.f58833it.hasNext()) {
                this.done = true;
                return null;
            }
            return ObjectHelper.requireNonNull(this.f58833it.next(), "The iterator returned a null value");
        }
