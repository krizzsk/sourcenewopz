...
public class Mat {
    ...
    public Mat(Mat mat, Rect rect) {
        this.nativeObj = n_Mat(mat.nativeObj, rect.f6690y, rect.height + rect.f6690y, rect.f6689x, rect.f6689x + rect.width);
    }
    ...
    public void locateROI(Size size, Point point) {
        ...
        if (point != null) {
            point.f6684x = dArr2[0];
            point.f6685y = dArr2[1];
        }
    }
    ...
    public Mat submat(Rect rect) {
        return new Mat(n_submat(this.nativeObj, rect.f6689x, rect.f6690y, rect.width, rect.height));
    }
