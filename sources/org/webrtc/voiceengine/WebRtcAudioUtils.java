...
public final class WebRtcAudioUtils {
    ...
    public static synchronized void setWebRtcBasedAutomaticGainControl(boolean z) {
        synchronized (WebRtcAudioUtils.class) {
            Logging.m3804w(TAG, "setWebRtcBasedAutomaticGainControl() is deprecated");
        }
    }
    ...
    public static synchronized boolean useWebRtcBasedAcousticEchoCanceler() {
        ...
        synchronized (WebRtcAudioUtils.class) {
            if (useWebRtcBasedAcousticEchoCanceler) {
                Logging.m3804w(TAG, "Overriding default behavior; now using WebRTC AEC!");
            }
    ...
    public static synchronized boolean useWebRtcBasedNoiseSuppressor() {
        ...
        synchronized (WebRtcAudioUtils.class) {
            if (useWebRtcBasedNoiseSuppressor) {
                Logging.m3804w(TAG, "Overriding default behavior; now using WebRTC NS!");
            }
    ...
    static void logDeviceInfo(String str) {
        Logging.m3800d(str, "Android SDK: " + Build.VERSION.SDK_INT + ", Release: " + Build.VERSION.RELEASE + ", Brand: " + Build.BRAND + ", Device: " + Build.DEVICE + ", Id: " + Build.ID + ", Hardware: " + Build.HARDWARE + ", Manufacturer: " + Build.MANUFACTURER + ", Model: " + Build.MODEL + ", Product: " + Build.PRODUCT);
    }
    ...
    private static void logAudioStateBasic(String str, AudioManager audioManager) {
        Logging.m3800d(str, "Audio State: audio mode: " + modeToString(audioManager.getMode()) + ", has mic: " + hasMicrophone() + ", mic muted: " + audioManager.isMicrophoneMute() + ", music active: " + audioManager.isMusicActive() + ", speakerphone: " + audioManager.isSpeakerphoneOn() + ", BT SCO: " + audioManager.isBluetoothScoOn());
    }
    ...
    private static void logAudioStateVolume(String str, AudioManager audioManager) {
        ...
        Logging.m3800d(str, "Audio State: ");
        boolean isVolumeFixed = isVolumeFixed(audioManager);
        Logging.m3800d(str, "  fixed volume=" + isVolumeFixed);
        if (!isVolumeFixed) {
            for (int i = 0; i < 6; i++) {
                int i2 = iArr[i];
                ...
                Logging.m3800d(str, sb.toString());
            }
        }
    }
    ...
    private static void logAudioDeviceInfo(String str, AudioManager audioManager) {
        if (Build.VERSION.SDK_INT >= 23) {
            ...
            if (devices.length != 0) {
                Logging.m3800d(str, "Audio Devices: ");
