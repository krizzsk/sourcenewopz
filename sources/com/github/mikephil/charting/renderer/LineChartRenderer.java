...
public class LineChartRenderer extends LineRadarRenderer {
    ...
    /* renamed from: a */
    private float[] f52473a = new float[4];

    /* renamed from: b */
    private HashMap<IDataSet, DataSetImageCache> f52474b = new HashMap<>();

    /* renamed from: c */
    private float[] f52475c = new float[2];
    ...
    public void drawDataSet(Canvas canvas, ILineDataSet iLineDataSet) {
        if (iLineDataSet.getEntryCount() >= 1) {
            ...
            int i = C178661.$SwitchMap$com$github$mikephil$charting$data$LineDataSet$Mode[iLineDataSet.getMode().ordinal()];
            if (i == 3) {
    ...
    static /* synthetic */ class C178661 {
        ...
        static {
            ...
            L_0x0033:
                return
            */
            throw new UnsupportedOperationException("Method not decompiled: com.github.mikephil.charting.renderer.LineChartRenderer.C178661.<clinit>():void");
        }
    }
    ...
    public void drawLinear(Canvas canvas, ILineDataSet iLineDataSet) {
        ...
        if (iLineDataSet.getColors().size() > 1) {
            int i2 = i * 2;
            if (this.f52473a.length <= i2) {
                this.f52473a = new float[(i * 4)];
            }
            for (int i3 = this.mXBounds.min; i3 <= this.mXBounds.range + this.mXBounds.min; i3++) {
                ...
                if (entryForIndex != null) {
                    this.f52473a[0] = entryForIndex.getX();
                    this.f52473a[1] = entryForIndex.getY() * phaseY;
                    if (i3 < this.mXBounds.max) {
                        Entry entryForIndex2 = iLineDataSet2.getEntryForIndex(i3 + 1);
                        if (entryForIndex2 == null) {
                            break;
                        } else if (z) {
                            this.f52473a[2] = entryForIndex2.getX();
                            float[] fArr = this.f52473a;
                            fArr[3] = fArr[1];
                            fArr[4] = fArr[2];
                            fArr[5] = fArr[3];
                            fArr[6] = entryForIndex2.getX();
                            this.f52473a[7] = entryForIndex2.getY() * phaseY;
                        } else {
                            this.f52473a[2] = entryForIndex2.getX();
                            this.f52473a[3] = entryForIndex2.getY() * phaseY;
                        }
                    } else {
                        float[] fArr2 = this.f52473a;
                        fArr2[2] = fArr2[0];
                        fArr2[3] = fArr2[1];
                    }
                    transformer.pointValuesToPixel(this.f52473a);
                    if (!this.mViewPortHandler.isInBoundsRight(this.f52473a[0])) {
                        break;
                    } else if (this.mViewPortHandler.isInBoundsLeft(this.f52473a[2]) && (this.mViewPortHandler.isInBoundsTop(this.f52473a[1]) || this.mViewPortHandler.isInBoundsBottom(this.f52473a[3]))) {
                        this.mRenderPaint.setColor(iLineDataSet2.getColor(i3));
                        canvas2.drawLines(this.f52473a, 0, i2, this.mRenderPaint);
                    }
                }
            }
        } else {
            int i4 = entryCount * i;
            if (this.f52473a.length < Math.max(i4, i) * 2) {
                this.f52473a = new float[(Math.max(i4, i) * 4)];
            }
            if (iLineDataSet2.getEntryForIndex(this.mXBounds.min) != null) {
                ...
                while (i5 <= this.mXBounds.range + this.mXBounds.min) {
                    ...
                    if (!(entryForIndex3 == null || entryForIndex4 == null)) {
                        int i7 = i6 + 1;
                        this.f52473a[i6] = entryForIndex3.getX();
                        int i8 = i7 + 1;
                        this.f52473a[i7] = entryForIndex3.getY() * phaseY;
                        if (z) {
                            int i9 = i8 + 1;
                            this.f52473a[i8] = entryForIndex4.getX();
                            int i10 = i9 + 1;
                            this.f52473a[i9] = entryForIndex3.getY() * phaseY;
                            int i11 = i10 + 1;
                            this.f52473a[i10] = entryForIndex4.getX();
                            i8 = i11 + 1;
                            this.f52473a[i11] = entryForIndex3.getY() * phaseY;
                        }
                        int i12 = i8 + 1;
                        this.f52473a[i8] = entryForIndex4.getX();
                        this.f52473a[i12] = entryForIndex4.getY() * phaseY;
                        i6 = i12 + 1;
                    }
                    i5++;
                }
                if (i6 > 0) {
                    transformer.pointValuesToPixel(this.f52473a);
                    this.mRenderPaint.setColor(iLineDataSet.getColor());
                    canvas2.drawLines(this.f52473a, 0, Math.max((this.mXBounds.range + 1) * i, i) * 2, this.mRenderPaint);
                }
            }
        }
        this.mRenderPaint.setPathEffect((PathEffect) null);
    ...
    public void drawLinearFill(Canvas canvas, ILineDataSet iLineDataSet, Transformer transformer, BarLineScatterCandleBubbleRenderer.XBounds xBounds) {
        ...
        do {
            ...
            if (i <= i2) {
                m37283a(iLineDataSet, i, i2, path);
                transformer.pathValueToPixel(path);
                Drawable fillDrawable = iLineDataSet.getFillDrawable();
                if (fillDrawable != null) {
                    drawFilledPath(canvas, path, fillDrawable);
    ...
    /* renamed from: a */
    private void m37283a(ILineDataSet iLineDataSet, int i, int i2, Path path) {
        float fillLinePosition = iLineDataSet.getFillFormatter().getFillLinePosition(iLineDataSet, this.mChart);
        float phaseY = this.mAnimator.getPhaseY();
        boolean z = iLineDataSet.getMode() == LineDataSet.Mode.STEPPED;
        path.reset();
                    ...
                    while (i4 < generateTransformedValuesLine.length) {
                        ...
                        if (!this.mViewPortHandler.isInBoundsLeft(f) || !this.mViewPortHandler.isInBoundsY(f2)) {
                            ...
                            if (entry.getIcon() != null && iLineDataSet.isDrawIconsEnabled()) {
                                ...
                                Utils.drawImage(canvas, icon, (int) (f + instance.f52498x), (int) (f2 + instance.f52499y), icon.getIntrinsicWidth(), icon.getIntrinsicHeight());
                            }
                        }
                        i4 += 2;
    ...
    public void drawCircles(Canvas canvas) {
        ...
        while (i < dataSets.size()) {
            ...
            if (iLineDataSet.isVisible() && iLineDataSet.isDrawCirclesEnabled() && iLineDataSet.getEntryCount() != 0) {
                ...
                if (this.f52474b.containsKey(iLineDataSet)) {
                    dataSetImageCache = this.f52474b.get(iLineDataSet);
                } else {
                    dataSetImageCache = new DataSetImageCache(this, (C178661) null);
                    this.f52474b.put(iLineDataSet, dataSetImageCache);
                }
                ...
                while (i3 <= i2) {
                    ...
                    this.f52475c[c] = entryForIndex.getX();
                    this.f52475c[1] = entryForIndex.getY() * phaseY;
                    transformer.pointValuesToPixel(this.f52475c);
                    if (!this.mViewPortHandler.isInBoundsRight(this.f52475c[c])) {
                        break;
                    }
                    if (!this.mViewPortHandler.isInBoundsLeft(this.f52475c[c]) || !this.mViewPortHandler.isInBoundsY(this.f52475c[1]) || (bitmap = dataSetImageCache.getBitmap(i3)) == null) {
                        Canvas canvas2 = canvas;
                    } else {
                        float[] fArr2 = this.f52475c;
        ...
        for (Highlight highlight : highlightArr) {
            ...
            if (iLineDataSet != null && iLineDataSet.isHighlightEnabled()) {
                ...
                if (isInBoundsX(entryForXValue, iLineDataSet)) {
                    ...
                    drawHighlightLines(canvas, (float) pixelForValues.f52495x, (float) pixelForValues.f52496y, iLineDataSet);
                }
            }
        }
    }
    ...
    private class DataSetImageCache {
        ...
        /* synthetic */ DataSetImageCache(LineChartRenderer lineChartRenderer, C178661 r2) {
            this();
        }
