...
public class MediaCodecVideoDecoder {
    ...
    static class HwDecoderFactory implements VideoDecoderFactory {
        ...
        private static VideoCodecInfo[] getSupportedHardwareCodecs() {
            ...
            if (MediaCodecVideoDecoder.isVp8HwSupported()) {
                Logging.m3800d(MediaCodecVideoDecoder.TAG, "VP8 HW Decoder supported.");
                arrayList.add(new VideoCodecInfo(RTCConsts.VIDEO_CODEC_VP8, new HashMap()));
            }
            if (MediaCodecVideoDecoder.isVp9HwSupported()) {
                Logging.m3800d(MediaCodecVideoDecoder.TAG, "VP9 HW Decoder supported.");
                arrayList.add(new VideoCodecInfo("VP9", new HashMap()));
            }
            if (MediaCodecVideoDecoder.isH264HighProfileHwSupported()) {
                Logging.m3800d(MediaCodecVideoDecoder.TAG, "H.264 High Profile HW Decoder supported.");
                arrayList.add(H264Utils.DEFAULT_H264_HIGH_PROFILE_CODEC);
            }
            if (MediaCodecVideoDecoder.isH264HwSupported()) {
                Logging.m3800d(MediaCodecVideoDecoder.TAG, "H.264 HW Decoder supported.");
        ...
        public VideoDecoder createDecoder(final VideoCodecInfo videoCodecInfo) {
            if (!isCodecSupported(this.supportedHardwareCodecs, videoCodecInfo)) {
                Logging.m3800d(MediaCodecVideoDecoder.TAG, "No HW video decoder for codec " + videoCodecInfo.name);
                return null;
            }
            Logging.m3800d(MediaCodecVideoDecoder.TAG, "Create HW video decoder for " + videoCodecInfo.name);
    ...
    public static void disableVp8HwCodec() {
        Logging.m3804w(TAG, "VP8 decoding is disabled by application.");
        hwDecoderDisabledTypes.add("video/x-vnd.on2.vp8");
    }

    public static void disableVp9HwCodec() {
        Logging.m3804w(TAG, "VP9 decoding is disabled by application.");
        hwDecoderDisabledTypes.add("video/x-vnd.on2.vp9");
    }

    public static void disableH264HwCodec() {
        Logging.m3804w(TAG, "H.264 decoding is disabled by application.");
    ...
    public static void printStackTrace() {
        ...
        if (mediaCodecVideoDecoder != null && (thread = mediaCodecVideoDecoder.mediaCodecThread) != null) {
            ...
            if (stackTrace.length > 0) {
                Logging.m3800d(TAG, "MediaCodecVideoDecoder stacks trace:");
    ...
    private static DecoderProperties findDecoder(String str, String[] strArr) {
        ...
        Logging.m3800d(TAG, "Trying to find HW decoder for mime " + str);
        for (int i = 0; i < MediaCodecList.getCodecCount(); i++) {
            try {
                mediaCodecInfo = MediaCodecList.getCodecInfoAt(i);
            } catch (IllegalArgumentException e) {
                Logging.m3802e(TAG, "Cannot retrieve decoder codec info", e);
                mediaCodecInfo = null;
            }
            if (mediaCodecInfo != null && !mediaCodecInfo.isEncoder()) {
                ...
                if (str2 == null) {
                    ...
                    Logging.m3800d(TAG, "Found candidate decoder " + str2);
                    ...
                    if (!z) {
                        ...
                        try {
                            ...
                            for (Integer intValue : supportedColorList) {
                                ...
                                while (true) {
                                    if (i5 < length3) {
                                        int i6 = iArr[i5];
                                        if (i6 == intValue2) {
                                            Logging.m3800d(TAG, "Found target decoder " + str2 + ". Color: 0x" + Integer.toHexString(i6));
                            ...
                            Logging.m3802e(TAG, "Cannot retrieve decoder capabilities", e2);
                        }
                    }
                }
            }
        }
        Logging.m3800d(TAG, "No HW decoder found for mime " + str);
        return null;
    }
    ...
    private boolean initDecode(VideoCodecType videoCodecType, int i, int i2) {
        ...
        if (this.mediaCodecThread == null) {
            ...
            if (findDecoder != null) {
                Logging.m3800d(TAG, "Java initDecode: " + videoCodecType + " : " + i + " x " + i2 + ". Color: 0x" + Integer.toHexString(findDecoder.colorFormat) + ". Use Surface: " + useSurface());
                ...
                try {
                    ...
                    Logging.m3800d(TAG, "Input buffers: " + this.inputBuffers.length + ". Output buffers: " + this.outputBuffers.length);
                    return true;
                } catch (IllegalStateException e) {
                    Logging.m3802e(TAG, "initDecode failed", e);
                    return false;
                }
            } else {
    ...
    private void reset(int i, int i2) {
        ...
        Logging.m3800d(TAG, "Java reset: " + i + " x " + i2);
    ...
    private void release() {
        Logging.m3800d(TAG, "Java releaseDecoder. Total number of dropped frames: " + this.droppedFrames);
        checkOnMediaCodecThread();
        final CountDownLatch countDownLatch = new CountDownLatch(1);
        new Thread(new Runnable() {
            public void run() {
                try {
                    Logging.m3800d(MediaCodecVideoDecoder.TAG, "Java releaseDecoder on release thread");
                    MediaCodecVideoDecoder.this.mediaCodec.stop();
                    MediaCodecVideoDecoder.this.mediaCodec.release();
                    Logging.m3800d(MediaCodecVideoDecoder.TAG, "Java releaseDecoder on release thread done");
                } catch (Exception e) {
                    Logging.m3802e(MediaCodecVideoDecoder.TAG, "Media decoder release failed", e);
                }
                countDownLatch.countDown();
            }
        }).start();
        if (!ThreadUtils.awaitUninterruptibly(countDownLatch, 5000)) {
            Logging.m3801e(TAG, "Media decoder release timeout");
            codecErrors++;
        ...
        Logging.m3800d(TAG, "Java releaseDecoder done");
    }
    ...
    private boolean queueInputBuffer(int i, int i2, long j, long j2, long j3) {
        ...
        try {
            ...
            Logging.m3802e(TAG, "decode failed", e);
            return false;
        }
    }
    ...
    private class TextureListener implements VideoSink {
        ...
        public void addBufferToRender(DecodedOutputBuffer decodedOutputBuffer) {
            if (this.bufferToRender == null) {
                ...
                Logging.m3801e(MediaCodecVideoDecoder.TAG, "Unexpected addBufferToRender() called while waiting for a texture.");
        ...
        public void onFrame(VideoFrame videoFrame) {
            synchronized (this.newFrameLock) {
                if (this.renderedBuffer == null) {
                    ...
                    Logging.m3801e(MediaCodecVideoDecoder.TAG, "Unexpected onFrame() called while already holding a texture.");
    ...
    private DecodedOutputBuffer dequeueOutputBuffer(int i) {
        ...
        while (true) {
            ...
            if (dequeueOutputBuffer == -3) {
                this.outputBuffers = this.mediaCodec.getOutputBuffers();
                Logging.m3800d(TAG, "Decoder output buffers changed: " + this.outputBuffers.length);
                if (this.hasDecodedFirstFrame) {
                    throw new RuntimeException("Unexpected output buffer change event.");
                }
            } else if (dequeueOutputBuffer == -2) {
                MediaFormat outputFormat = this.mediaCodec.getOutputFormat();
                Logging.m3800d(TAG, "Decoder format changed: " + outputFormat.toString());
                ...
                if (!this.hasDecodedFirstFrame || (i2 == this.width && i3 == this.height)) {
                    ...
                    if (!useSurface() && outputFormat.containsKey("color-format")) {
                        ...
                        Logging.m3800d(TAG, "Color: 0x" + Integer.toHexString(this.colorFormat));
                    ...
                    Logging.m3800d(TAG, "Frame stride and slice height: " + this.stride + " x " + this.sliceHeight);
                ...
                if (elapsedRealtime > 200) {
                    Logging.m3801e(TAG, "Very high decode time: " + elapsedRealtime + "ms. Q size: " + this.decodeStartTimeMs.size() + ". Might be caused by resuming H264 decoding after a pause.");
                    j = 200;
                } else {
                    j = elapsedRealtime;
                }
    ...
    private DecodedTextureBuffer dequeueTextureBuffer(int i) {
        ...
        if (useSurface()) {
            ...
            if (dequeueTextureBuffer != null) {
                ...
                if (i2 > 0) {
                    Logging.m3804w(TAG, "Draining decoder. Dropping frame with TS: " + remove.presentationTimeStampMs + ". Total number of dropped frames: " + this.droppedFrames);
                } else {
                    Logging.m3804w(TAG, "Too many output buffers " + this.dequeuedSurfaceOutputBuffers.size() + ". Dropping frame with TS: " + remove.presentationTimeStampMs + ". Total number of dropped frames: " + this.droppedFrames);
                }
