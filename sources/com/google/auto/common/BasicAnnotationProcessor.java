...
public abstract class BasicAnnotationProcessor extends AbstractProcessor {
    ...
    private String processingErrorMessage(String str) {
        return String.format("[%s:MiscError] %s was unable to process %s because not all of its dependencies could be resolved. Check for compilation errors or a circular dependency with generated code.", new Object[]{getClass().getSimpleName(), getClass().getCanonicalName(), str});
    }
    ...
    private ImmutableSetMultimap<Class<? extends Annotation>, Element> validElements(ImmutableMap<String, Optional<? extends Element>> immutableMap, RoundEnvironment roundEnvironment) {
        ...
        while (it2.hasNext()) {
            ...
            if (typeElement == null) {
                set = ImmutableSet.m38415of();
            } else {
                set = roundEnvironment.getElementsAnnotatedWith(typeElement);
            }
            UnmodifiableIterator it3 = Sets.union(set, build.get((Object) cls)).iterator();
            while (it3.hasNext()) {
                ...
                if (packageElement.getKind().equals(ElementKind.PACKAGE)) {
                    ...
                    if (z) {
                        builder2.put((Object) cls, (Object) packageElement2);
                    ...
                    if (z) {
                        builder2.put((Object) cls, (Object) packageElement);
    ...
    private void process(ImmutableSetMultimap<Class<? extends Annotation>, Element> immutableSetMultimap) {
        ...
        while (it.hasNext()) {
            ...
            ImmutableSetMultimap build = new ImmutableSetMultimap.Builder().putAll((Multimap) indexByAnnotation(this.elementsDeferredBySteps.get(processingStep))).putAll((Multimap) Multimaps.filterKeys(immutableSetMultimap, Predicates.m38348in(processingStep.annotations()))).build();
            if (build.isEmpty()) {
                this.elementsDeferredBySteps.removeAll(processingStep);
            } else {
                this.elementsDeferredBySteps.replaceValues(processingStep, Iterables.transform(processingStep.process(build), new Function<Element, ElementName>() {
    ...
    private static void findAnnotatedElements(Element element, ImmutableSet<? extends Class<? extends Annotation>> immutableSet, ImmutableSetMultimap.Builder<Class<? extends Annotation>, Element> builder) {
        ...
        while (it.hasNext()) {
            ...
            if (MoreElements.isAnnotationPresent(element, cls)) {
                builder.put((Object) cls, (Object) element);
            }
        }
    }
    ...
    public static TypeElement getEnclosingType(Element element) {
        return (TypeElement) element.accept(new SimpleElementVisitor6<TypeElement, Void>() {
            ...
            public TypeElement visitPackage(PackageElement packageElement, Void voidR) {
