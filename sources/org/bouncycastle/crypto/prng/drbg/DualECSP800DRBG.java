...
public class DualECSP800DRBG implements SP80090DRBG {
    ...
    /* renamed from: _P */
    private ECPoint f6207_P;

    /* renamed from: _Q */
    private ECPoint f6208_Q;
    private ECCurve.C2746Fp _curve;
    ...
    /* renamed from: _s */
    private byte[] f6209_s;
    private int _sLength;
    private int _securityStrength;
    private int _seedlen;

    static {
        ECCurve.C2746Fp fp = (ECCurve.C2746Fp) NISTNamedCurves.getByName("P-256").getCurve();
        nistPoints[0] = new DualECPoints(128, fp.createPoint(p256_Px, p256_Py), fp.createPoint(p256_Qx, p256_Qy), 1);
        ECCurve.C2746Fp fp2 = (ECCurve.C2746Fp) NISTNamedCurves.getByName("P-384").getCurve();
        nistPoints[1] = new DualECPoints(192, fp2.createPoint(p384_Px, p384_Py), fp2.createPoint(p384_Qx, p384_Qy), 1);
        ECCurve.C2746Fp fp3 = (ECCurve.C2746Fp) NISTNamedCurves.getByName("P-521").getCurve();
        ...
        if (Utils.isTooLarge(bArr, 512)) {
            ...
            while (true) {
                if (i2 == dualECPointsArr.length) {
                    ...
                    this.f6207_P = dualECPointsArr[i2].getP();
                    this.f6208_Q = dualECPointsArr[i2].getQ();
                } else {
                    throw new IllegalArgumentException("Requested security strength is not supported by digest");
                }
            }
            if (this.f6207_P != null) {
                byte[] hash_df = Utils.hash_df(this._digest, concatenate, this._seedlen);
                this.f6209_s = hash_df;
    ...
    public int generate(byte[] bArr, byte[] bArr2, boolean z) {
        ...
        if (Utils.isTooLarge(bArr2, 512)) {
            ...
            if (bArr2 != null) {
                bigInteger = new BigInteger(1, xor(this.f6209_s, Utils.hash_df(this._digest, bArr2, this._seedlen)));
            } else {
                bigInteger = new BigInteger(1, this.f6209_s);
            }
            int i = 0;
            Arrays.fill(bArr, (byte) 0);
            int i2 = 0;
            for (int i3 = 0; i3 < length2; i3++) {
                bigInteger = getScalarMultipleXCoord(this.f6207_P, bigInteger);
                byte[] byteArray = getScalarMultipleXCoord(this.f6208_Q, bigInteger).toByteArray();
            ...
            if (i2 < bArr.length) {
                bigInteger = getScalarMultipleXCoord(this.f6207_P, bigInteger);
                byte[] byteArray2 = getScalarMultipleXCoord(this.f6208_Q, bigInteger).toByteArray();
            ...
            this.f6209_s = BigIntegers.asUnsignedByteArray(this._sLength, getScalarMultipleXCoord(this.f6207_P, bigInteger));
            return length;
        }
    }
    ...
    public void reseed(byte[] bArr) {
        if (!Utils.isTooLarge(bArr, 512)) {
            this.f6209_s = Utils.hash_df(this._digest, Arrays.concatenate(pad8(this.f6209_s, this._seedlen), getEntropy(), bArr), this._seedlen);
            this._reseedCounter = 0;
            return;
        }
