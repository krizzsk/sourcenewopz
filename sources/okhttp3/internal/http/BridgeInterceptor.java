...
public final class BridgeInterceptor implements Interceptor {
    ...
    /* renamed from: a */
    private final CookieJar f5351a;

    public BridgeInterceptor(CookieJar cookieJar) {
        this.f5351a = cookieJar;
    }
    ...
    public Response intercept(Interceptor.Chain chain) throws IOException {
        ...
        if (request.header(HttpHeaders.HOST) == null) {
            newBuilder.header(HttpHeaders.HOST, C2434Util.hostHeader(request.url(), false));
        }
        ...
        List<Cookie> loadForRequest = this.f5351a.loadForRequest(request.url());
        if (!loadForRequest.isEmpty()) {
            newBuilder.header(HttpHeaders.COOKIE, m3431a(loadForRequest));
        }
        if (request.header("User-Agent") == null) {
            newBuilder.header("User-Agent", Version.userAgent());
        }
        Response proceed = chain.proceed(newBuilder.build());
        HttpHeaders.receiveHeaders(this.f5351a, request.url(), proceed.headers());
    ...
    /* renamed from: a */
    private String m3431a(List<Cookie> list) {
