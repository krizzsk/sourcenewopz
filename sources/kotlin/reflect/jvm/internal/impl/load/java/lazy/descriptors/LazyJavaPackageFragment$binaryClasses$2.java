...
final class LazyJavaPackageFragment$binaryClasses$2 extends Lambda implements Function0<Map<String, ? extends KotlinJvmBinaryClass>> {
    ...
    public final Map<String, KotlinJvmBinaryClass> invoke() {
        PackagePartProvider packagePartProvider = this.this$0.f60550c.getComponents().getPackagePartProvider();
        ...
        for (String str : packagePartProvider.findPackageParts(asString)) {
            ...
            KotlinJvmBinaryClass findKotlinClass = KotlinClassFinderKt.findKotlinClass(lazyJavaPackageFragment.f60550c.getComponents().getKotlinClassFinder(), classId);
            Pair pair = findKotlinClass == null ? null : TuplesKt.m42317to(str, findKotlinClass);
            if (pair != null) {
                arrayList.add(pair);
            }
        }
