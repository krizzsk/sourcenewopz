...
public class YAxisRendererRadarChart extends YAxisRenderer {
    ...
    /* renamed from: a */
    private RadarChart f52490a;

    /* renamed from: b */
    private Path f52491b = new Path();

    public YAxisRendererRadarChart(ViewPortHandler viewPortHandler, YAxis yAxis, RadarChart radarChart) {
        super(viewPortHandler, yAxis, (Transformer) null);
        this.f52490a = radarChart;
    }
    ...
    public void renderAxisLabels(Canvas canvas) {
        if (this.mYAxis.isEnabled() && this.mYAxis.isDrawLabelsEnabled()) {
            ...
            MPPointF centerOffsets = this.f52490a.getCenterOffsets();
            MPPointF instance = MPPointF.getInstance(0.0f, 0.0f);
            float factor = this.f52490a.getFactor();
            int i = this.mYAxis.isDrawTopYLabelEntryEnabled() ? this.mYAxis.mEntryCount : this.mYAxis.mEntryCount - 1;
            for (int i2 = !this.mYAxis.isDrawBottomYLabelEntryEnabled(); i2 < i; i2++) {
                Utils.getPosition(centerOffsets, (this.mYAxis.mEntries[i2] - this.mYAxis.mAxisMinimum) * factor, this.f52490a.getRotationAngle(), instance);
                canvas.drawText(this.mYAxis.getFormattedLabel(i2), instance.f52498x + 10.0f, instance.f52499y, this.mAxisLabelPaint);
            }
    ...
    public void renderLimitLines(Canvas canvas) {
        ...
        if (limitLines != null) {
            float sliceAngle = this.f52490a.getSliceAngle();
            float factor = this.f52490a.getFactor();
            MPPointF centerOffsets = this.f52490a.getCenterOffsets();
            MPPointF instance = MPPointF.getInstance(0.0f, 0.0f);
            for (int i = 0; i < limitLines.size(); i++) {
                ...
                if (limitLine.isEnabled()) {
                    ...
                    float limit = (limitLine.getLimit() - this.f52490a.getYChartMin()) * factor;
                    Path path = this.f52491b;
                    path.reset();
                    for (int i2 = 0; i2 < ((IRadarDataSet) ((RadarData) this.f52490a.getData()).getMaxEntryCountSet()).getEntryCount(); i2++) {
                        Utils.getPosition(centerOffsets, limit, (((float) i2) * sliceAngle) + this.f52490a.getRotationAngle(), instance);
                        if (i2 == 0) {
