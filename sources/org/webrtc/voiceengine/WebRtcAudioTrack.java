...
public class WebRtcAudioTrack {
    ...
    public static synchronized void setAudioTrackUsageAttribute(int i) {
        synchronized (WebRtcAudioTrack.class) {
            Logging.m3804w(TAG, "Default usage attribute is changed from: " + DEFAULT_USAGE + " to " + i);
            usageAttribute = i;
        }
    }
    ...
    public static void setErrorCallback(WebRtcAudioTrackErrorCallback webRtcAudioTrackErrorCallback) {
        Logging.m3800d(TAG, "Set error callback (deprecated");
        errorCallbackOld = webRtcAudioTrackErrorCallback;
    }

    public static void setErrorCallback(ErrorCallback errorCallback2) {
        Logging.m3800d(TAG, "Set extended error callback");
    ...
    private class AudioTrackThread extends Thread {
        ...
        public void run() {
            ...
            Logging.m3800d(WebRtcAudioTrack.TAG, "AudioTrackThread" + WebRtcAudioUtils.getThreadInfo());
            ...
            while (this.keepAlive) {
                ...
                if (writeBytes != capacity) {
                    Logging.m3801e(WebRtcAudioTrack.TAG, "AudioTrack.write played invalid number of bytes: " + writeBytes);
                    if (writeBytes < 0) {
            ...
            if (WebRtcAudioTrack.this.audioTrack != null) {
                Logging.m3800d(WebRtcAudioTrack.TAG, "Calling AudioTrack.stop...");
                try {
                    WebRtcAudioTrack.this.audioTrack.stop();
                    Logging.m3800d(WebRtcAudioTrack.TAG, "AudioTrack.stop is done.");
                } catch (IllegalStateException e) {
                    Logging.m3801e(WebRtcAudioTrack.TAG, "AudioTrack.stop failed: " + e.getMessage());
                }
            }
        }
        ...
        public void stopThread() {
            Logging.m3800d(WebRtcAudioTrack.TAG, "stopThread");
            this.keepAlive = false;
        }
    }
    ...
    private boolean initPlayout(int i, int i2, double d) {
        ...
        Logging.m3800d(TAG, "initPlayout(sampleRate=" + i + ", channels=" + i2 + ", bufferSizeFactor=" + d + ")");
        ...
        Logging.m3800d(TAG, "minBufferSizeInBytes: " + minBufferSize);
    ...
    private boolean startPlayout() {
        this.threadChecker.checkIsOnValidThread();
        Logging.m3800d(TAG, "startPlayout");
    ...
    private boolean stopPlayout() {
        this.threadChecker.checkIsOnValidThread();
        Logging.m3800d(TAG, "stopPlayout");
        assertTrue(this.audioThread != null);
        logUnderrunCount();
        this.audioThread.stopThread();
        Logging.m3800d(TAG, "Stopping the AudioTrackThread...");
        this.audioThread.interrupt();
        if (!ThreadUtils.joinUninterruptibly(this.audioThread, AUDIO_TRACK_THREAD_JOIN_TIMEOUT_MS)) {
            Logging.m3801e(TAG, "Join of AudioTrackThread timed out.");
            WebRtcAudioUtils.logAudioState(TAG);
        }
        Logging.m3800d(TAG, "AudioTrackThread has now been stopped.");
    ...
    private int getStreamMaxVolume() {
        this.threadChecker.checkIsOnValidThread();
        Logging.m3800d(TAG, "getStreamMaxVolume");
    ...
    private boolean setStreamVolume(int i) {
        this.threadChecker.checkIsOnValidThread();
        Logging.m3800d(TAG, "setStreamVolume(" + i + ")");
        assertTrue(this.audioManager != null);
        if (isVolumeFixed()) {
            Logging.m3801e(TAG, "The device implements a fixed volume policy.");
            return false;
        }
    ...
    private int getStreamVolume() {
        this.threadChecker.checkIsOnValidThread();
        Logging.m3800d(TAG, "getStreamVolume");
        assertTrue(this.audioManager != null);
        return this.audioManager.getStreamVolume(0);
    }

    private void logMainParameters() {
        Logging.m3800d(TAG, "AudioTrack: session ID: " + this.audioTrack.getAudioSessionId() + ", channels: " + this.audioTrack.getChannelCount() + ", sample rate: " + this.audioTrack.getSampleRate() + ", max gain: " + AudioTrack.getMaxVolume());
    }

    private static AudioTrack createAudioTrackOnLollipopOrHigher(int i, int i2, int i3) {
        Logging.m3800d(TAG, "createAudioTrackOnLollipopOrHigher");
        int nativeOutputSampleRate = AudioTrack.getNativeOutputSampleRate(0);
        Logging.m3800d(TAG, "nativeOutputSampleRate: " + nativeOutputSampleRate);
        if (i != nativeOutputSampleRate) {
            Logging.m3804w(TAG, "Unable to use fast mode since requested sample rate is not native");
        }
        if (usageAttribute != DEFAULT_USAGE) {
            Logging.m3804w(TAG, "A non default usage attribute is used: " + usageAttribute);
        }
    ...
    private void logBufferSizeInFrames() {
        if (Build.VERSION.SDK_INT >= 23) {
            Logging.m3800d(TAG, "AudioTrack: buffer size in frames: " + this.audioTrack.getBufferSizeInFrames());
        }
    }

    private void logBufferCapacityInFrames() {
        if (Build.VERSION.SDK_INT >= 24) {
            Logging.m3800d(TAG, "AudioTrack: buffer capacity in frames: " + this.audioTrack.getBufferCapacityInFrames());
        }
    }
    ...
    private void logUnderrunCount() {
        if (Build.VERSION.SDK_INT >= 24) {
            Logging.m3800d(TAG, "underrun count: " + this.audioTrack.getUnderrunCount());
        }
    }
    ...
    public static void setSpeakerMute(boolean z) {
        Logging.m3804w(TAG, "setSpeakerMute(" + z + ")");
        speakerMute = z;
    }

    private void releaseAudioResources() {
        Logging.m3800d(TAG, "releaseAudioResources");
        AudioTrack audioTrack2 = this.audioTrack;
        if (audioTrack2 != null) {
            audioTrack2.release();
    ...
    private void reportWebRtcAudioTrackInitError(String str) {
        Logging.m3801e(TAG, "Init playout error: " + str);
        WebRtcAudioUtils.logAudioState(TAG);
    ...
    private void reportWebRtcAudioTrackStartError(AudioTrackStartErrorCode audioTrackStartErrorCode, String str) {
        Logging.m3801e(TAG, "Start playout error: " + audioTrackStartErrorCode + ". " + str);
        WebRtcAudioUtils.logAudioState(TAG);
        WebRtcAudioTrackErrorCallback webRtcAudioTrackErrorCallback = errorCallbackOld;
        if (webRtcAudioTrackErrorCallback != null) {
    ...
    public void reportWebRtcAudioTrackError(String str) {
        Logging.m3801e(TAG, "Run-time playback error: " + str);
        WebRtcAudioUtils.logAudioState(TAG);
        WebRtcAudioTrackErrorCallback webRtcAudioTrackErrorCallback = errorCallbackOld;
        if (webRtcAudioTrackErrorCallback != null) {
