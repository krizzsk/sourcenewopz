...
final class AndroidObjectInspectors$MORTAR_PRESENTER$inspect$1 extends Lambda implements Function2<ObjectReporter, HeapObject.HeapInstance, Unit> {
    ...
    public final void invoke(ObjectReporter objectReporter, HeapObject.HeapInstance heapInstance) {
        ...
        if (heapField.getValue().isNullReference()) {
            objectReporter.getLeakingReasons().add(AndroidObjectInspectorsKt.m2905a(heapField, "null"));
        } else {
            objectReporter.getLabels().add(AndroidObjectInspectorsKt.m2905a(heapField, GuideParamModel.ACTION_SET));
        }
    }
}
