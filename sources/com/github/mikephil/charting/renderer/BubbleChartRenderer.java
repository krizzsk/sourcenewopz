...
public class BubbleChartRenderer extends BarLineScatterCandleBubbleRenderer {
    ...
    /* renamed from: a */
    private float[] f52458a = new float[4];

    /* renamed from: b */
    private float[] f52459b = new float[2];

    /* renamed from: c */
    private float[] f52460c = new float[3];
    ...
    public void drawDataSet(Canvas canvas, IBubbleDataSet iBubbleDataSet) {
        if (iBubbleDataSet.getEntryCount() >= 1) {
            ...
            for (int i = this.mXBounds.min; i <= this.mXBounds.range + this.mXBounds.min; i++) {
                ...
                this.f52459b[0] = bubbleEntry.getX();
                this.f52459b[1] = bubbleEntry.getY() * phaseY;
                transformer.pointValuesToPixel(this.f52459b);
                float shapeSize = getShapeSize(bubbleEntry.getSize(), iBubbleDataSet.getMaxSize(), min, isNormalizeSizeEnabled) / 2.0f;
                if (this.mViewPortHandler.isInBoundsTop(this.f52459b[1] + shapeSize) && this.mViewPortHandler.isInBoundsBottom(this.f52459b[1] - shapeSize) && this.mViewPortHandler.isInBoundsLeft(this.f52459b[0] + shapeSize)) {
                    if (this.mViewPortHandler.isInBoundsRight(this.f52459b[0] - shapeSize)) {
                        this.mRenderPaint.setColor(iBubbleDataSet.getColor((int) bubbleEntry.getX()));
                        float[] fArr3 = this.f52459b;
                ...
                if (bubbleEntry.getY() == highlight.getY() && isInBoundsX(bubbleEntry, iBubbleDataSet)) {
                    ...
                    if (this.mViewPortHandler.isInBoundsTop(this.f52459b[1] + shapeSize) && this.mViewPortHandler.isInBoundsBottom(this.f52459b[1] - shapeSize) && this.mViewPortHandler.isInBoundsLeft(this.f52459b[0] + shapeSize)) {
                        if (this.mViewPortHandler.isInBoundsRight(this.f52459b[0] - shapeSize)) {
                            ...
                            Color.RGBToHSV(Color.red(color), Color.green(color), Color.blue(color), this.f52460c);
                            float[] fArr4 = this.f52460c;
                            fArr4[2] = fArr4[2] * 0.5f;
                            this.mHighlightPaint.setColor(Color.HSVToColor(Color.alpha(color), this.f52460c));
                            this.mHighlightPaint.setStrokeWidth(iBubbleDataSet.getHighlightCircleWidth());
                            float[] fArr5 = this.f52459b;
