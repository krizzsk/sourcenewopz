...
import com.google.p217ar.core.exceptions.AnchorNotSupportedForHostingException;
import com.google.p217ar.core.exceptions.CameraNotAvailableException;
import com.google.p217ar.core.exceptions.CloudAnchorsNotConfiguredException;
import com.google.p217ar.core.exceptions.DataInvalidFormatException;
import com.google.p217ar.core.exceptions.DataUnsupportedVersionException;
import com.google.p217ar.core.exceptions.DeadlineExceededException;
import com.google.p217ar.core.exceptions.FatalException;
import com.google.p217ar.core.exceptions.ImageInsufficientQualityException;
import com.google.p217ar.core.exceptions.MissingGlContextException;
import com.google.p217ar.core.exceptions.NotTrackingException;
import com.google.p217ar.core.exceptions.NotYetAvailableException;
import com.google.p217ar.core.exceptions.ResourceExhaustedException;
import com.google.p217ar.core.exceptions.SessionNotPausedException;
import com.google.p217ar.core.exceptions.SessionPausedException;
import com.google.p217ar.core.exceptions.TextureNotSetException;
...
import com.google.p217ar.core.exceptions.UnsupportedConfigurationException;
...
public class Session {
    ...
    enum C18632a {
        ...
        /* renamed from: k */
        final Class<? extends Exception> f53481k;

        /* renamed from: l */
        final String f53482l;

        private C18632a(int i, Class<? extends Exception> cls) {
            this(r7, r8, i, cls, (String) null);
        }

        private C18632a(int i, Class<? extends Exception> cls, String str) {
            this.f53480j = i;
            this.f53481k = cls;
            this.f53482l = str;
        }
    ...
    enum C18634c {
        ...
        /* renamed from: h */
        private final Class<?> f53494h;

        /* renamed from: a */
        public abstract Trackable mo149595a(long j, Session session);

        private C18634c(int i, Class<? extends Trackable> cls) {
            this.f53493b = i;
            this.f53494h = cls;
        }

        /* renamed from: a */
        public static C18634c m38270a(Class<? extends Trackable> cls) {
            for (C18634c cVar : values()) {
                Class<?> cls2 = cVar.f53494h;
    ...
    public Session(Context context, Set<Feature> set) throws UnavailableArcoreNotInstalledException, UnavailableApkTooOldException, UnavailableSdkTooOldException, UnavailableDeviceNotCompatibleException, IllegalArgumentException {
        ...
        iArr[i] = C18633b.END_OF_LIST.f53485b;
    ...
    public Trackable createTrackable(long j) {
        ...
        while (true) {
            ...
            cVar = values[i];
            if (cVar.f53493b == internalGetType) {
                break;
            }
            i++;
        }
        if (cVar != null) {
            return cVar.mo149595a(j, this);
        }
    ...
    public <T extends Trackable> Collection<T> getAllTrackables(Class<T> cls) {
        C18634c a = C18634c.m38270a(cls);
        if (a == C18634c.UNKNOWN_TO_JAVA) {
            return Collections.emptyList();
        }
        return convertNativeTrackablesToCollection(cls, nativeAcquireAllTrackables(this.nativeWrapperHandle, a.f53493b));
    }
    ...
    static void throwExceptionFromArStatus(int i) throws Exception {
        C18632a[] values = C18632a.values();
        int length = values.length;
        int i2 = 0;
        while (i2 < length) {
            C18632a aVar = values[i2];
            if (aVar.f53480j != i) {
                i2++;
            } else if (aVar.f53481k == null) {
                return;
            } else {
                if (aVar.f53482l != null) {
                    throw ((Exception) aVar.f53481k.getConstructor(new Class[]{String.class}).newInstance(new Object[]{aVar.f53482l}));
                }
                throw ((Exception) aVar.f53481k.getConstructor(new Class[0]).newInstance(new Object[0]));
            }
        }
