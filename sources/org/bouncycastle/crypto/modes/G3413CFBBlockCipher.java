...
public class G3413CFBBlockCipher extends StreamBlockCipher {
    ...
    private void initArrays() {
        int i = this.f6089m;
        this.f6088R = new byte[i];
        this.R_init = new byte[i];
    }

    private void setupDefaultParams() {
        this.f6089m = this.blockSize * 2;
    }
    ...
    public byte[] createGamma() {
        byte[] MSB = GOST3413CipherUtil.MSB(this.f6088R, this.blockSize);
        byte[] bArr = new byte[MSB.length];
        this.cipher.processBlock(MSB, 0, bArr, 0);
        return GOST3413CipherUtil.MSB(bArr, this.f6090s);
    }

    /* access modifiers changed from: package-private */
    public void generateR(byte[] bArr) {
        byte[] LSB = GOST3413CipherUtil.LSB(this.f6088R, this.f6089m - this.f6090s);
        System.arraycopy(LSB, 0, this.f6088R, 0, LSB.length);
        System.arraycopy(bArr, 0, this.f6088R, LSB.length, this.f6089m - LSB.length);
    }
    ...
    public void init(boolean z, CipherParameters cipherParameters) throws IllegalArgumentException {
        ...
        if (cipherParameters instanceof ParametersWithIV) {
            ...
            if (iv.length >= this.blockSize) {
                this.f6089m = iv.length;
                initArrays();
                byte[] clone = Arrays.clone(iv);
                this.R_init = clone;
                System.arraycopy(clone, 0, this.f6088R, 0, clone.length);
        ...
        System.arraycopy(bArr, 0, this.f6088R, 0, bArr.length);
