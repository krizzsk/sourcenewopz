...
abstract class CameraCapturer implements CameraVideoCapturer {
    ...
    public final CameraSession.Events cameraSessionEventsHandler = new CameraSession.Events() {
        public void onCameraOpening() {
            ...
            synchronized (CameraCapturer.this.stateLock) {
                if (CameraCapturer.this.currentSession != null) {
                    Logging.m3804w(CameraCapturer.TAG, "onCameraOpening while session was open.");
                } else {
        ...
        public void onCameraError(CameraSession cameraSession, String str) {
            ...
            synchronized (CameraCapturer.this.stateLock) {
                if (cameraSession != CameraCapturer.this.currentSession) {
                    Logging.m3804w(CameraCapturer.TAG, "onCameraError from another session: " + str);
                    return;
                }
        ...
        public void onCameraDisconnected(CameraSession cameraSession) {
            ...
            synchronized (CameraCapturer.this.stateLock) {
                if (cameraSession != CameraCapturer.this.currentSession) {
                    Logging.m3804w(CameraCapturer.TAG, "onCameraDisconnected from another session.");
                    return;
                }
        ...
        public void onCameraClosed(CameraSession cameraSession) {
            ...
            synchronized (CameraCapturer.this.stateLock) {
                if (cameraSession == CameraCapturer.this.currentSession || CameraCapturer.this.currentSession == null) {
                    ...
                    Logging.m3800d(CameraCapturer.TAG, "onCameraClosed from another session.");
                }
            }
        }
        ...
        public void onFrameCaptured(CameraSession cameraSession, VideoFrame videoFrame) {
            ...
            synchronized (CameraCapturer.this.stateLock) {
                if (cameraSession != CameraCapturer.this.currentSession) {
                    Logging.m3804w(CameraCapturer.TAG, "onFrameCaptured from another session.");
                    return;
                }
    ...
    public final CameraSession.CreateSessionCallback createSessionCallback = new CameraSession.CreateSessionCallback() {
        public void onDone(CameraSession cameraSession) {
            CameraCapturer.this.checkIsOnCameraThread();
            Logging.m3800d(CameraCapturer.TAG, "Create session done. Switch state: " + CameraCapturer.this.switchState);
        ...
        public void onFailure(CameraSession.FailureType failureType, String str) {
            ...
            synchronized (CameraCapturer.this.stateLock) {
                ...
                if (CameraCapturer.this.openAttemptsRemaining <= 0) {
                    Logging.m3804w(CameraCapturer.TAG, "Opening camera failed, passing: " + str);
                    boolean unused = CameraCapturer.this.sessionOpening = false;
                    CameraCapturer.this.stateLock.notifyAll();
                    ...
                    Logging.m3804w(CameraCapturer.TAG, "Opening camera failed, retry: " + str);
    ...
    public void startCapture(int i, int i2, int i3) {
        Logging.m3800d(TAG, "startCapture: " + i + "x" + i2 + "@" + i3);
        if (this.applicationContext != null) {
            synchronized (this.stateLock) {
                ...
                Logging.m3804w(TAG, "Session already open");
                return;
            }
        }
    ...
    public void stopCapture() {
        Logging.m3800d(TAG, "Stop capture");
        synchronized (this.stateLock) {
            while (this.sessionOpening) {
                Logging.m3800d(TAG, "Stop capture: Waiting for session to open");
                try {
                    this.stateLock.wait();
                } catch (InterruptedException unused) {
                    Logging.m3804w(TAG, "Stop capture interrupted while waiting for the session to open.");
                    Thread.currentThread().interrupt();
                    return;
                }
            }
            if (this.currentSession != null) {
                Logging.m3800d(TAG, "Stop capture: Nulling session");
                ...
                Logging.m3800d(TAG, "Stop capture: No session open");
            }
        }
        Logging.m3800d(TAG, "Stop capture done");
    }

    public void changeCaptureFormat(int i, int i2, int i3) {
        Logging.m3800d(TAG, "changeCaptureFormat: " + i + "x" + i2 + "@" + i3);
        synchronized (this.stateLock) {
            stopCapture();
    ...
    public void dispose() {
        Logging.m3800d(TAG, "dispose");
        stopCapture();
    }

    public void switchCamera(final CameraVideoCapturer.CameraSwitchHandler cameraSwitchHandler) {
        Logging.m3800d(TAG, "switchCamera");
        this.cameraThreadHandler.post(new Runnable() {
            public void run() {
    ...
    public void printStackTrace() {
        ...
        if (thread != null) {
            ...
            if (stackTrace.length > 0) {
                Logging.m3800d(TAG, "CameraCapturer stack trace:");
    ...
    public void switchCameraInternal(CameraVideoCapturer.CameraSwitchHandler cameraSwitchHandler) {
        Logging.m3800d(TAG, "switchCamera internal");
        ...
        if (deviceNames.length >= 2) {
            synchronized (this.stateLock) {
                if (this.switchState != SwitchState.IDLE) {
                    ...
                    Logging.m3800d(TAG, "switchCamera: Stopping session");
                    this.cameraStatistics.release();
                    this.cameraStatistics = null;
                    final CameraSession cameraSession = this.currentSession;
                    this.cameraThreadHandler.post(new Runnable() {
                    ...
                    createSessionInternal(0);
                    Logging.m3800d(TAG, "switchCamera done");
                } else {
    ...
    public void checkIsOnCameraThread() {
        if (Thread.currentThread() != this.cameraThreadHandler.getLooper().getThread()) {
            Logging.m3801e(TAG, "Check is on camera thread failed.");
